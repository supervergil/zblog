<template>
  <div class="component-z-upload-picker" v-loading="configging" element-loading-text="配置加载中...">
    ...
    <el-tabs type="card" v-model="active">
      <el-tab-pane v-for="item in type" :key="item" :label="tabKey[item].name" :name="item">
        <div
          ...
          v-loading="tabKey[item].loading || fileLoading"
        ...
        >
          ...
          <div v-if="tabKey[item].data.length<0">暂无数据</div>
  ...
  methods: {
    ...
    async uploadFiles(param) {
      ...
      if (this.uploadType === 0) {
        await services.common.upload({
          contentType: "multipart/form-data",
          data: form,
          warning: true
        });
      } else if (this.uploadType === 1) {
        await services.common.uploadToQiniu({
          contentType: "multipart/form-data",
          data: form,
          warning: true
        });
    ...
    async getAllList() {
      ...
      await Promise.all(this.type.map(item => this.getSpec(item)));
