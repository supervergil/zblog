  ...
  <div :class="{'component-z-md-editor':true,'fullscreen':fullScreen}">
    <div class="component-z-md-editor__tool">
      ...
      <div class="component-z-md-editor__tool-right">
        ...
        <span :class="{'tool-item':true,'active':alignCopy==='left'}" @click="toggleAlign('left')">
          <i class="fa-fw fas fa-chevron-right"></i>
        </span>
        <span
          :class="{'tool-item':true,'active':alignCopy==='center'}"
          @click="toggleAlign('center')"
        >
          <i class="fa-fw fas fa-columns"></i>
        </span>
        <span
          :class="{'tool-item':true,'active':alignCopy==='right'}"
    ...
    <div :class="['component-z-md-editor__container',alignCopy]">
...
export default {
  ...
  data() {
    return {
      ...
      fullScreen: false,
      alignCopy: this.align,
      markdownCopy: this.markdown
    };
  },
  computed: {
    richtext() {
  ...
  watch: {
    // 解决markdown异步传输问题
    markdown: function(newValue) {
      this.markdownCopy = newValue;
    }
  },
  methods: {
    hideResourceDialog() {
    ...
    insertResource() {
      ...
      if (resource.extension.startsWith("image")) {
        insertion = `<img src="${resource.url}" title="${resource.name}" alt="${resource.name}"  style="width: 320px;" />`;
      } else if (resource.extension.startsWith("video")) {
        insertion = `<video src="${resource.url}" controls width="100%"></video>`;
      } else if (resource.extension.startsWith("audio")) {
        insertion = `<audio src="${resource.url}" controls width="100%"></audio>`;
      }
      insertTextAtCursor(
        this.$refs["markdown"].$el.querySelector("textarea"),
        insertion
    ...
    toggleAlign(align) {
      this.alignCopy = align;
    },
    emitMarkDown() {
      this.$emit("input", this.markdownCopy);
    },
    $getRichtext() {
