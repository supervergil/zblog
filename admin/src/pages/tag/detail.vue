<template>
  <div class="page-tag-detail">
    ...
    <el-form
      ...
      element-loading-text="正在保存标签..."
      class="page-tag-detail__form"
    >
  ...
  methods: {
    save() {
      this.$refs["form"].validate(async valid => {
        if (valid) {
          ...
          const { status } = await services.tag.save({
    ...
    async remove() {
      try {
        ...
        const { status } = await services.tag.remove({
          id: this.$route.params.id
        });
        if (status === 1) {
    ...
    async getDetail() {
      const { status, data } = await services.tag.getDetail({
        id: this.$route.params.id
      });
      if (status === 1) {
