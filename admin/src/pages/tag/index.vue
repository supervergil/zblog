<template>
  <div class="page-tag">
    <h2>
      标签管理
      <router-link to="/tag/add" class="page-tag__btn-add">
        <el-button type="success" size="mini">新增标签</el-button>
      </router-link>
    </h2>
    <el-table :data="tData" v-loading="loading" element-loading-text="数据加载中..." stripe>
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <router-link :to="'/tag/'+scope.row.id" class="page-tag__btn">
    ...
    <div class="page-tag__pagination">
      <el-pagination
        background
...
export default {
  ...
  methods: {
    async remove(id) {
      try {
        await this.$confirm("是否删除该标签?", "提示", {
          confirmButtonText: "删除",
          cancelButtonText: "取消",
          type: "warning"
        });
        const { status } = await services.tag.remove({ id });
        if (status === 1) {
    ...
    async getList() {
      this.loading = true;
      const { status, data } = await services.tag.getList({
