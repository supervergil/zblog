<template>
  <div class="page-page-detail">
    <el-form
      ...
      align="center"
      label-position="top"
      class="page-page-detail__form"
    >
      <el-form-item prop="title">
        <el-input v-model="form.title" placeholder="页面标题"></el-input>
      </el-form-item>
      <el-form-item prop="pathname">
        <el-input v-model="form.pathname" :disabled="true" placeholder="页面路径">
      ...
      <el-form-item prop="cover">
        <div class="page-page-detail__picker" @click="showCoverDialog" v-if="form.cover===''">
          <div class="text">页面封面 (推荐 220*100)</div>
        </div>
        <div class="page-page-detail__cover" @click="showCoverDialog" v-else>
          <div
...
export default {
  ...
  methods: {
    save(published) {
      this.$refs["form"].validate(async valid => {
        if (valid) {
          ...
          const { status } = await services.page.save({
            id: this.form.id,
    ...
    async getPage() {
      const { status, data } = await services.page.getDetail({
      ...
      if (status === 1) {
        ...
        this.form.description = data.meta_description;
        this.form.keywords = data.meta_keywords.split(",");
        this.form.markdown = data.markdown;
        this.form.styleList = data.style_list
          .split(",")
        ...
        this.form.styleCustom = data.style_custom;
        this.form.scriptList = data.script_list
          .split(",")
          .filter(item => item.length > 0)
          .map(item => {
            return {
        ...
        this.form.scriptCustom = data.script_custom;
        this.form.layout = data.layout;
        this.form.cover = data.cover_url;
        this.form.allowComment = data.allow_comment === 1;
