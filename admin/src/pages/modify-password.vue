    ...
    save() {
      this.$refs["form"].validate(async valid => {
        if (valid) {
          ...
          const { status } = await services.author.modifyPassword({
            originPassword: md5(
              `${this.$store.state.user.account}${this.form.origin}:zblog`
            ).toString(),
            newPassword: md5(
              `${this.$store.state.user.account}${this.form.new}:zblog`
            ).toString(),
            confirmPassword: md5(
              `${this.$store.state.user.account}${this.form.confirm}:zblog`
            ).toString()
          });
          if (status === 1) {
